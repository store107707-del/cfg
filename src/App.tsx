...
const Hero = ({ onRegister, liveSettings }: { onRegister: () => void, liveSettings: any }) => (
      ...
      >
        ...
        {liveSettings.isLive && (
          <div className="mb-10 p-6 bg-white/[0.03] border-l-4 border-bs-red inline-flex items-center gap-8 font-display font-black text-2xl md:text-4xl uppercase italic tracking-tighter">
            ...
            <span className="text-bs-cyan">{liveSettings.teamA}</span>
            <span className="text-bs-red text-5xl not-italic">VS</span>
            <span className="text-bs-cyan">{liveSettings.teamB}</span>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-6 items-center">
          ...
          {liveSettings.showLiveButton ? (
            <a 
              href={liveSettings.liveUrl}
              target="_blank"
              rel="noreferrer"
...
const DetailView = ({ tournament, onCancel, onRegister }: { tournament: any, onCancel: () => void, onRegister: () => void }) => {
  const theme = TOURNAMENT_THEMES.find(t => t.id === tournament.theme) || TOURNAMENT_THEMES[0];
...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (result.user.email === 'sihumin196@gmail.com') {
        setIsAdminOpen(true);
      } else {
        ...
        <div className="max-w-7xl mx-auto mt-32 pt-16 border-t-2 border-white/5 flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="flex flex-col items-start gap-4">
            <div 
              ...
              className="w-10 h-10 cursor-default opacity-0 hover:opacity-10 transition-opacity"
              title="SYSTEM ACCESS"
            />
            <p className="text-[10px] font-black uppercase text-white/20 tracking-[0.5em]">© 2026 BRALWSTARS ARENA. 모든 권리 보유.</p>
          </div>
          <div className="flex gap-8 text-[10px] font-black uppercase text-white/20 tracking-tighter italic">
            <span>{currentUser?.email === 'sihumin196@gmail.com' ? 'ADMIN AUTHORIZED' : 'GUEST SESSION'}</span>
